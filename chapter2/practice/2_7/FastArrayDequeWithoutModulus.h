...
namespace data_structure {
  ...
  void FastArrayDeque<T>::add(int i, T x) {
    ...
    if (2 * i < n) {
      ...
      fastcopy_backward((j+n-1) & (a.length-1), n-i);
    }
    a[(j + i) & (a.length-1)] = x;
    n++;
  }
  ...
  T FastArrayDeque<T>::remove(int i) {
    T x = a[(j + i) & (a.length-1)];
    if (2 * i < n) {
      fastcopy_backward((j+i-1) & (a.length-1), i);
      j = (j + 1) & (a.length-1);
    } else {
      fastcopy((j+i+1) & (a.length-1), n-i-1);
    }
    n--;
    // 常にlengthが２のべき乗になるようにresizeのタイミングを調整する
    // resize()のコストに関してはp30と同様の証明が成り立つ　R >= ni/3-1 ならば ΣO(ni) <= O(3m+3r) = O(m)
    if (4 * n <= a.length) resize();
    return x;
  }
  ...
  void FastArrayDeque<T>::resize() {
    ...
    if (copy_last != j+n)
      copy(a+0, a + ((j+n) & (a.length-1)), b+(copy_last-j));
